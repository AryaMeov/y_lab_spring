...
import com.edu.ulab.app.exception.PersonValidationException;
...
public class UserBookRequestValidationService {
    public void validateCreateRequest(UserBookRequest request) {
        ...
        if (request.getUserRequest().getId() != null) {
            throw new PersonValidationException("User id in create request is not empty");
        }
    ...
    public void validateUpdateRequest(UserBookRequest request) {
        validateRequest(request);
        if (request.getUserRequest().getId() == null) {
            throw new PersonValidationException("User id in update request is empty");
        }
        if (request.getUserRequest().getId() < 0) {
            throw new PersonValidationException("User id in update request is negative");
        }
        validateUserRequest(request.getUserRequest());
        request.getBookRequests().forEach(book -> {
            if (book == null) {
    ...
    private void validateRequest(UserBookRequest request) {
        ...
        if (request.getUserRequest() == null) {
            throw new PersonValidationException("User in request is empty");
        }
    ...
    private void validateUserRequest(UserRequest request) {
        if (request.getFullName() == null || request.getFullName().isBlank()) {
            throw new PersonValidationException("User full name in request is empty");
        }
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new PersonValidationException("User title in request is empty");
        }
        if (request.getAge() < 0) {
            throw new PersonValidationException("User age in request is negative");
        }
    }
